...
- [x] 1. Extend configuration with fallback settings
  - [x] 1.1 Add FallbackConfig struct to `internal/config/config.go`
  ...
  - [x] 1.2 Write unit tests for fallback config defaults
...
- [x] 2. Extend xray config generation in `internal/tunnel/configgen.go`
  - [x] 2.1 Implement full xray outbound generation for trojan and shadowsocks
  ...
  - [x] 2.2 Implement xray streamSettings builder
  ...
  - [x] 2.3 Add unsupported protocol error for xray
    - Return clear error for hysteria2, tuic, wireguard, openvpn protocols in `generateXray`
    - _Requirements: 3.3_

  - [x] 2.4 Write property tests for xray config generation
...
- [x] 3. Checkpoint - Ensure config generation tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Implement FallbackController in `internal/engine/fallback.go`
  - [x] 4.1 Create FallbackController struct and StartWithFallback method
  ...
  - [x] 4.2 Implement failure detection logic
  ...
  - [x] 4.3 Implement fallback logging
  ...
  - [x] 4.4 Write property tests for FallbackController
...
- [x] 5. Integrate fallback into Gateway (`internal/gateway/gateway.go`)
  - [x] 5.1 Refactor `startEngine` to use FallbackController
  ...
  - [x] 5.2 Add active engine name to Gateway status
  ...
  - [x] 5.3 Write unit tests for gateway fallback integration
...
- [x] 6. Update default config and documentation
  - [x] 6.1 Add fallback section to `configs/default.yaml`
